...
- BETA tab:
  * Combat.SetGloveTpSlap -> Glove TP Slap toggle
...
function Items.IsMeteorCrate(object)
	...
	local current = object
	local fullName = ""
	local hasMeteorMarker = false
	local hasCrateMarker = false

	while current and current ~= workspace do
		local normalizedName = Utility.NormalizeName(current.Name)
		fullName = fullName .. " " .. normalizedName

		if string.find(normalizedName, "meteor") then
			hasMeteorMarker = true
		end

		if string.find(normalizedName, "crate") then
			hasCrateMarker = true
		end

		pcall(function()
			for _, tagName in ipairs(Services.CollectionService:GetTags(current)) do
				local normalizedTagName = Utility.NormalizeName(tagName)

				if string.find(normalizedTagName, "meteor") then
					hasMeteorMarker = true
				end

				if string.find(normalizedTagName, "crate") then
					hasCrateMarker = true
				end
			end
		end)
		...
		pcall(function()
			for attributeName, attributeValue in pairs(current:GetAttributes()) do
				local normalizedAttributeName = Utility.NormalizeName(attributeName)
				local normalizedAttributeValue = Utility.NormalizeName(tostring(attributeValue))

				if string.find(normalizedAttributeName, "meteor") or string.find(normalizedAttributeValue, "meteor") then
					hasMeteorMarker = true
				end

				if string.find(normalizedAttributeName, "crate") or string.find(normalizedAttributeValue, "crate") then
					hasCrateMarker = true
				end
			end
		end)
	...
	if string.find(fullName, "crater")
		or string.find(fullName, "food")
		or string.find(fullName, "regular")
		or string.find(fullName, "ground")
		or string.find(fullName, "hole")
		or string.find(fullName, "debris")
		or string.find(fullName, "effect")
		or string.find(fullName, "shockwave")
		or string.find(fullName, "explosion") then
		return false
	end

	if not hasMeteorMarker or not hasCrateMarker then
		return false
	end
...
function Items.GetCrateRoot(object)
	...
	while current and current ~= workspace do
		if string.find(Utility.NormalizeName(current.Name), "crater") then
			return nil
		end
...
function Items.GetCrateUnderMapCFrame(cratePart)
	local partSize = cratePart.Size
	local bottomY = cratePart.Position.Y - (partSize.Y / 2)
	local targetPosition = Vector3.new(cratePart.Position.X, bottomY - 3, cratePart.Position.Z)

	return CFrame.new(targetPosition, cratePart.Position)
end

function Items.EnsureCrateCollectPart(targetCFrame)
	local part = Items.CrateCollectPart
	...
	part.CFrame = CFrame.new(targetCFrame.Position - Vector3.new(0, 8, 0))
...
function Items.CollectCrates()
	if Items.CollectCratesBusy then
		createNotification("Collect Crates", "Already collecting crates.", "Warning")
		return
	end
	...
	local crates = table.clone(Items.Crates)
	if #crates == 0 then
		createNotification("Collect Crates", "No meteor crates spawned yet.", "Warning")
		return
	end
	...
	task.spawn(function()
		...
		local ok, err = pcall(function()
			...
			if not root then
				createNotification("Collect Crates", "Could not find your character.", "Error")
				return
			end

			for _, crate in ipairs(crates) do
				local cratePart = Items.GetCratePart(crate)

				if cratePart then
					local targetCFrame = Items.GetCrateUnderMapCFrame(cratePart)

					Items.EnsureCrateCollectPart(targetCFrame)
					Teleport.MoveRoot(root, targetCFrame, cratePart.Position)
					task.wait(0.12)

					for _ = 1, Items.CrateCollectSlaps do
						if not cratePart.Parent then
							break
						end

						if Items.SlapCrate(cratePart) then
							slapCount += 1
						end

						task.wait(Items.CrateCollectSlapInterval)
					end
		...
		if not ok then
			createNotification("Collect Crates", tostring(err), "Error")
		elseif slapCount <= 0 then
			createNotification("Collect Crates", "Could not find the Slap remote.", "Error")
		elseif collected > 0 then
			Teleport.AddStrike()
			Teleport.StartFBlock()
			createNotification("Collect Crates", "Slapped " .. tostring(collected) .. " meteor crate(s).", "Success")
		end
...
function Items.StartCrateWatcher()
	if Items.CrateWatcherStarted then
		return
	end
	...
	table.insert(Items.CrateWatcherConnections, root.DescendantAdded:Connect(function(object)
		task.defer(function()
...
local AUTO_PERMANENT_USE_DEBOUNCE = 0.12
local AUTO_PERMANENT_SCAN_INTERVAL = 0.12
...
function Items.DisableEarlyAutoCollectConflicts(silent)
	Items.EarlyAutoCollectRestoreAutoPickup = false
	Items.EarlyAutoCollectRestoreAutoPermanent =
		autoPermanentEnabled == true or (autoPermanentToggle and autoPermanentToggle.Get and autoPermanentToggle.Get() == true) or false
	...
	if Items.EarlyAutoCollectRestoreAutoPermanent then
		...
		if not silent then
			createNotification("Early Auto Collect", "Auto Use Permanent Items disabled.", "Info")
		end
	end
...
function Items.RestoreEarlyAutoCollectConflicts(silent)
	local restoreAutoPermanent = Items.EarlyAutoCollectRestoreAutoPermanent == true

	Items.EarlyAutoCollectRestoreAutoPickup = false
	Items.EarlyAutoCollectRestoreAutoPermanent = false

	if restoreAutoPermanent then
		if autoPermanentToggle then
		...
		if not silent then
			createNotification("Auto Use", "Auto Use Permanent Items restored.", "Info")
		end
	end
end
...
function Items.StopPriorityCollect(mode, message, notInLobby)
	...
	Items.RestoreEarlyAutoCollectConflicts(true)
...
function Items.SetEarlyAutoCollect(state, silent)
	if state then
		...
		Items.DisableEarlyAutoCollectConflicts(silent)
		...
		Items.EarlyAutoCollectEnabled = false
		setMovementPaused(false)
		Items.RestoreEarlyAutoCollectConflicts(silent)
		if not silent then
...
function Combat.CanUseSlapAuraTarget(targetPlayer)
	...
	if targetCharacter:GetAttribute("Ragdolled") ~= false or localCharacter:GetAttribute("Ragdolled") ~= false then
		return nil, nil
	end
...
local WindUIWindow = WindUI:CreateWindow({
	...
	Transparent = true,
	Theme = "Dark", -- tema zdsdasd javjsdbj
	SideBarWidth = 200,
})
...
local TabMovement = WindUIWindow:Tab({ Title = "Movement", Icon = "footprints" })
...
local function SetAntiVotekick(state)
    getgenv().noVotekick = state

    if state then
        task.spawn(function()
            while getgenv().noVotekick do
                local usrnme = game.Players.LocalPlayer.Name
...
function UI.TeleportAutoRejoin(reason)
	...
	createNotification("Auto Rejoin", "Finding fullest server after " .. tostring(reason) .. ".", "Info")
	...
	task.spawn(function()
		...
		if serverId then
			createNotification("Auto Rejoin", "Joining fullest server (" .. tostring(playing) .. "/" .. tostring(maxPlayers) .. ").", "Info")
		...
		if not joinedServer then
			createNotification("Auto Rejoin", "Joining place " .. tostring(placeId) .. ".", "Info")
			pcall(function()
...
function UI.GetRecommendedFeatureState(key)
	...
	elseif key == "GloveTpSlap" then
		return Combat.GloveTpSlapEnabled == true
...
function UI.SetRecommendedSettings(state, silent)
	...
	if enabled then
		setToggle("AutoPermanentItems", true, true, { CheckFeatureState = false, MaxAttempts = 1 })
		setToggle("EarlyBusJump", true, false, { CheckFeatureState = false })
		UI.SetAutoEarlyBusJump(true, true)
		setToggle("EarlyAutoCollect", true, true, { CheckFeatureState = false })

		task.delay(0.5, function()
			if Items.EarlyAutoCollectEnabled then
				setToggle("AutoPermanentItems", false, false, { CheckFeatureState = false, MaxAttempts = 1 })
			else
				setToggle("AutoPermanentItems", true)
			end
		end)
...
function UI.GetMovementHumanoid()
	local character = player.Character
	return character and character:FindFirstChildOfClass("Humanoid")
end

function UI.GetMovementSpeedMax()
	local humanoid = UI.GetMovementHumanoid()
	local speed = humanoid and tonumber(humanoid.WalkSpeed) or UI.Movement.SpeedMin
	UI.Movement.SpeedMax = math.max(UI.Movement.SpeedMin, math.floor((speed or UI.Movement.SpeedMin) + 0.5))
	return UI.Movement.SpeedMax
end

function UI.GetMovementJumpMax()
	local humanoid = UI.GetMovementHumanoid()
	local jump = humanoid and tonumber(humanoid.JumpPower) or UI.Movement.JumpMin
	UI.Movement.JumpMax = math.max(UI.Movement.JumpMin, math.floor((jump or UI.Movement.JumpMin) + 0.5))
	return UI.Movement.JumpMax
end

function UI.SetMovementSpeed(value)
	local humanoid = UI.GetMovementHumanoid()
	local speed = math.clamp(tonumber(value) or UI.Movement.SpeedMin, UI.Movement.SpeedMin, UI.Movement.SpeedMax)

	if humanoid then
		UI.Movement.Applying = true
		humanoid.WalkSpeed = speed
		UI.Movement.LastObservedSpeed = speed
		UI.Movement.Applying = false
	end
end

function UI.SetMovementJump(value)
	local humanoid = UI.GetMovementHumanoid()
	local jump = math.clamp(tonumber(value) or UI.Movement.JumpMin, UI.Movement.JumpMin, UI.Movement.JumpMax)

	if humanoid then
		UI.Movement.Applying = true
		humanoid.JumpPower = jump
		UI.Movement.LastObservedJump = jump
		UI.Movement.Applying = false
	end
end

function UI.SyncMovementSlider(title, minValue, maxValue, value)
	local slider = _windSliders[title]

	if not slider then
		return
	end

	if type(slider.SetMin) == "function" then
		pcall(function()
			slider:SetMin(minValue)
		end)
	end

	if type(slider.SetMax) == "function" then
		pcall(function()
			slider:SetMax(maxValue)
		end)
	end

	if type(slider.Set) == "function" then
		pcall(function()
			slider:Set(value)
		end)
...
function UI.DisconnectMovement()
	for _, connection in ipairs(UI.Movement.Connections) do
		if connection then
			pcall(function()
				connection:Disconnect()
			end)
	...
	table.clear(UI.Movement.Connections)
end

function UI.ApplySavedMovement()
	if Preferences.Sliders.MovementSpeed ~= nil then
		UI.SetMovementSpeed(Preferences.Sliders.MovementSpeed)
	end

	if Preferences.Sliders.MovementJump ~= nil then
		UI.SetMovementJump(Preferences.Sliders.MovementJump)
	end
end

function UI.HookMovementHumanoid(character)
	UI.DisconnectMovement()
	...
	UI.Movement.LastObservedSpeed = tonumber(humanoid.WalkSpeed) or UI.Movement.SpeedMin
	UI.Movement.LastObservedJump = tonumber(humanoid.JumpPower) or UI.Movement.JumpMin
	UI.GetMovementSpeedMax()
	UI.GetMovementJumpMax()
	UI.ApplySavedMovement()

	table.insert(UI.Movement.Connections, humanoid:GetPropertyChangedSignal("WalkSpeed"):Connect(function()
		if UI.Movement.Applying then
			return
		end

		local speed = tonumber(humanoid.WalkSpeed) or UI.Movement.SpeedMin
		local lastSpeed = tonumber(UI.Movement.LastObservedSpeed) or speed

		UI.Movement.LastObservedSpeed = speed

		if math.abs(speed - lastSpeed - 20) < 0.001 then
			return
		end

		if speed > UI.Movement.SpeedMax then
			UI.Movement.SpeedMax = math.floor(speed + 0.5)
		end

		UI.SyncMovementSlider("Speed", UI.Movement.SpeedMin, UI.Movement.SpeedMax, speed)
	end))

	table.insert(UI.Movement.Connections, humanoid:GetPropertyChangedSignal("JumpPower"):Connect(function()
		if UI.Movement.Applying then
			return
		end

		local jump = tonumber(humanoid.JumpPower) or UI.Movement.JumpMin
		UI.Movement.LastObservedJump = jump

		if jump > UI.Movement.JumpMax then
			UI.Movement.JumpMax = math.floor(jump + 0.5)
		end

		UI.SyncMovementSlider("Jump", UI.Movement.JumpMin, UI.Movement.JumpMax, jump)
	end))
end

UI.Movement.CharacterConnection = player.CharacterAdded:Connect(function(character)
	task.defer(function()
		UI.HookMovementHumanoid(character)
	end)
end)

task.defer(function()
	UI.HookMovementHumanoid(player.Character)
end)

-- Main
AddButton(TabMain, "Get Code + Go Barn", "Teleports to Barn and scans for puzzle code", "hash", Main.GetCodeGoBarn)
...
AddToggle(TabMain, "Auto Rejoin", "Rejoins after death or disconnect", false, UI.SetAutoRejoin, "AutoRejoin")
AddToggle(TabMain, "Recommended Settings", "Slap Aura, ESP, auto item tools, early collect, anti-ragdoll, sorting, and item menu", false, UI.SetRecommendedSettings, "RecommendedSettings")
...
UI.GetMovementSpeedMax()
UI.GetMovementJumpMax()
AddSlider(TabMovement, "Speed", "Adjust walk speed", UI.Movement.SpeedMin, UI.Movement.SpeedMax, UI.Movement.SpeedMax, 1, UI.SetMovementSpeed, "MovementSpeed")
AddSlider(TabMovement, "Jump", "Adjust jump power", UI.Movement.JumpMin, UI.Movement.JumpMax, UI.Movement.JumpMax, 1, UI.SetMovementJump, "MovementJump")
...
-- BETA
AddToggle(TabBETA, "Glove TP Slap", "Teleports glove to nearest player on click", false, Combat.SetGloveTpSlap, "GloveTpSlap")
AddButton(TabBETA, "Collect Crates", "Teleports under meteor crates and slaps them", "box", Items.CollectCrates)
...
AddToggle(TabSafety, "Anti-Ragdoll / Anti-Slap", "Invisible cage on knockback", false, Combat.SetAntiSlap, "AntiRagdoll")
AddToggle(TabSafety, "Anti Votekick", "Prevents vote kick attempts", false, SetAntiVotekick, "AntiVotekick")
...
if sharedEnvironment then
	sharedEnvironment.OPSlapRoyaleCleanup = function()
		...
		pcall(function() Combat.SetGloveTpSlap(false) end)
		...
		pcall(function()
			UI.DisconnectMovement()
			if UI.Movement.CharacterConnection then
				UI.Movement.CharacterConnection:Disconnect()
				UI.Movement.CharacterConnection = nil
			end
		end)
...
if currentPlaceId == 9426795465 then
	createNotification(
    "R3O",
    "Match Making, Loading assets before game starts...",
    nil,
    "check",
    15,
    false
...
queue_on_teleport(loadstring(game:HttpGet("https://raw.githubusercontent.com/NYXosu/SlapBattlesHUB/refs/heads/main/R30"))())
